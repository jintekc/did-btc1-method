...
* Create
  * idType = key (default)
    ...
    * options.mnemonic = non-null, valid, mnemonic recovery phrase => use mnemonic to generate keys, did and did doc
  * idType = external
    * 
...
* Questions
  * Why set 'verificationMethod.type' to 'SchnorrSecp256k1VerificationKey2024' in resolution if we set it to 'JsonWebKey' during create?
  * Should we consider renaming "sidecar" to something more semantic and descriptive like "offline" or "local"?

------------

* @params did; @returns didDoc at time of resolution
...
* Read Algorithm
  * split did on colon (:) into: scheme, method, version, network and id
  * set defaults in case missing (implied) such as version = 1, network = mainnet
  * if included and not 1, cast version to int; same for network but no cast
  * do checks:
    * scheme must = "did"
    * method must = "btc1"
    * version must convert to pos int
    * network must be one of mainnet, signet, testnet, or regtest
    * on error, raise InvalidDID error
  * decode method specific id usin bech32-decode algo
  * if hrp = k, id-bech32 encodes a Secp256k1 public key used to deterministically generate a DID document
  * if hrp = x, then id-bech32 encodes an external initiating DID document, use 4.2.2 Ext Resolution to construct and return
  * else raise InvalidHRPValue error
